...
public record RegisterRequestDTO(
        ...
        @NotBlank(message = "Password is required") @Size(min = 8, message = "Password must be at least 8 characters") @Pattern(regexp = "^(?=.*[A-Z])(?=.*[0-9]).+$", message = "Password must contain at least one uppercase letter and one number") String password,

        @NotBlank(message = "BSN is required") @Pattern(regexp = "^[0-9]{8,9}$", message = "BSN must be 8 or 9 digits") String bsn,

        @Pattern(regexp = "^(\\+?[0-9]{10,15})?$", message = "Invalid phone number format") String phoneNumber) {
}
